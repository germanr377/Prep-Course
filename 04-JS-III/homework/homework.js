...
function numeroMasGrande(numeros) {
  ...
  for (i=1; i<numeros.length;i++) {if (numeros[i]>numerogrande){numerogrande=numeros[i]}}
return numerogrande
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  var multi = 1;
 for (var i=0;i<arguments.length;i++){ multi=arguments[i]*multi}
return multi;
}
...
for (var i=0; i<10;i++)
  {numero=numero+2;
    ...
    if (numero===i){return("Se interrumpió la ejecución")} else {array.push(numero)}
    }
return array
...
function continueStatement(numero) {
  ...
  for (var i=0; i<10;i++)

    {
      ...
      if (i !==5 ) {numero=numero+2; array.push(numero)} else {continue}
      }
